    ...
    <main style={{ fontFamily: "'Apple SD Gothic Neo', 'Noto Sans KR', sans-serif", backgroundColor: '#FAF7F2' }} className="text-gray-900">
      ...
      <section className="min-h-screen flex flex-col items-center justify-center text-center px-6 pt-20">
        ...
        <div className="mt-24 flex gap-16 pt-12" style={{ borderTop: '1px solid #E8DDD6' }}>
          {[
            { num: '6가지', label: '건강 유형별 맞춤 분석' },
            { num: '식약처', label: '인증 건강기능식품 연동' },
            { num: '1포', label: '하루 한 포 정기배송' },
          ].map((item) => (
      ...
      <section id="about" className="py-32 px-6" style={{ backgroundColor: '#F2EBE3' }}>
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { num: '01', icon: '🏥', title: '건강 상태 분석', desc: '간단한 설문으로 부모님의 건강 유형을 파악하고 꼭 필요한 영양소를 찾아드립니다' },
      ...
      <section id="mbti" className="py-32 px-6" style={{ backgroundColor: '#F2EBE3' }}>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-2 gap-5 mb-10">
            {[
              { type: '피로누적형', emoji: '😴', desc: '항상 피곤하고 기운이 없는 유형' },
              { type: '수면부족형', emoji: '😪', desc: '잠을 자도 개운하지 않은 유형' },
              { type: '다이어트형', emoji: '🥗', desc: '체중 관리가 고민인 유형' },
              { type: '면역강화형', emoji: '🛡️', desc: '잔병치레가 잦은 유형' },
              { type: '관절케어형', emoji: '🦵', desc: '무릎과 관절이 아픈 유형' },
              { type: '혈관건강형', emoji: '❤️', desc: '혈압과 콜레스테롤이 걱정인 유형' },
            ].map((item) => (
